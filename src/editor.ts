...
export function computeLabelEdit(block: MermaidBlock, nodeId: string, newLabel: string): EditResult {
  ...
  // typed callers — schema validation or the webview panel's coercion already guard this.)
  ...
  if (!node) {
    ...
    // — there is no label to edit until it has a shape. The read API reports such an id
...
export function computeIdRename(
  ...
  // unreachable via typed callers — schema validation / the panel.)
  ...
  // ever advancing lastIndex → an infinite loop. A try/catch wrapper only
  // catches throws, so that loop would hang the caller. Reject oldId symmetrically.
  if (!ID_RE.test(oldId)) {
