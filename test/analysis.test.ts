...
test('duplicates: a tag used as an edge ref AND a subgraph id is NOT flagged', () => {
  // The FrameworkFlow case: `A` is an edge endpoint and a subgraph, but never a
  // bracketed node — that is a legal edge-to-subgraph reference, not a collision.
  const { block, lines } = build(['graph LR', 'A --> B[op]', 'subgraph A [open ritual]', 'AA1[x]', 'end']);
  assert.deepEqual(findDuplicateDeclarations(block, lines), []);
});

// --- regressions from the adversarial FrameworkFlow sweep (all confirmed bugs) ---
...
// regression: an id rename must not rewrite an id-word inside a glued inline
...
// regression: an id rename must not corrupt a CSS value inside a style line.
